      ...
      <table className="w-full table-fixed border-collapse text-[13px]">
        <thead className="sticky top-0 z-10" style={{ background: 'var(--bg)' }}>
          <tr className="border-b">
            {/*
              * A phone cannot hold eight columns, and a table you have to drag
              * sideways is not a table anybody reads. The ones that answer
              * "what is it, where is it, whose is it" stay; the rest step out
              * below each breakpoint and are still on the task itself.
              */}
            <Th className="w-7 sm:w-9" />
            <Th sortable onClick={() => toggleSort('title')} sort={sort} k="title">Task</Th>
            <Th className="w-[92px] sm:w-[132px]" sortable onClick={() => toggleSort('status')} sort={sort} k="status">Status</Th>
            <Th className="hidden w-[118px] lg:table-cell" sortable onClick={() => toggleSort('priority')} sort={sort} k="priority">Priority</Th>
            <Th className="w-[96px] sm:w-[168px]" sortable onClick={() => toggleSort('assignee')} sort={sort} k="assignee">Assignee</Th>
            <Th className="hidden w-[112px] md:table-cell" sortable onClick={() => toggleSort('due')} sort={sort} k="due">Due</Th>
            <Th className="hidden w-[128px] xl:table-cell">Tags</Th>
            <Th className="hidden w-[86px] lg:table-cell">Activity</Th>
          </tr>
        </thead>
    ...
    <tr className="group border-b transition-colors hover:bg-[var(--bg-hover)]">
      ...
      <td className="min-w-0 px-2 py-1.5">
        ...
        >
          ...
          <span className="hidden shrink-0 text-[11px] text-[var(--text-tertiary)] sm:inline">TSK-{task.seq}</span>
